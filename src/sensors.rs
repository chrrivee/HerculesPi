...
impl SensorManager {
    ...
    fn find_supported_sensor(&self, api: &HidApi) -> Result<HidDevice, SensorError> {
        ...
        for &(vendor_id, product_id, description) in &supported_sensors {
            debug!("Looking for sensor: {} ({:04x}:{:04x})", description, vendor_id, product_id);
        ...
        for device_info in api.device_list() {
            ...
            let manufacturer = device_info.manufacturer_string().unwrap_or("").to_lowercase();
            
            if product.contains("gyro") || product.contains("accel") || 
               product.contains("imu") || product.contains("motion") ||
               manufacturer.contains("gyro") || manufacturer.contains("accel") {
                
                info!("Found potential IMU device: {} from {}", 
                     device_info.product_string().unwrap_or("Unknown"),
                     device_info.manufacturer_string().unwrap_or("Unknown"));
